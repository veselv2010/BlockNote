...
const blockOuter = "_blockOuter_1ltqa_5";
const block = "_block_1ltqa_5";
const blockContent = "_blockContent_1ltqa_19";
const blockGroup = "_blockGroup_1ltqa_47";
const isEmpty = "_isEmpty_1ltqa_232";
const isFilter = "_isFilter_1ltqa_233";
const hasAnchor = "_hasAnchor_1ltqa_246";
const blockStyles = {
  blockOuter,
  block,
  blockContent,
...
class FormattingToolbarView {
  ...
  getSelectionBoundingBox() {
    ...
    if (isNodeSelection(selection)) {
      const node = this.editor.view.nodeDOM(from2);
      if (node) {
        return node.getBoundingClientRect();
      }
    }
    return posToDOMRect(this.editor.view, from2, to);
  }
  getStaticParams() {
    return {
      toggleBold: () => {
      ...
      setHyperlink: (url, text2) => {
        ...
        let { from: from2, to } = this.editor.state.selection;
        if (!text2) {
          text2 = this.editor.state.doc.textBetween(from2, to);
        }
        const mark = this.editor.schema.mark("link", { href: url });
        this.editor.view.dispatch(
          this.editor.view.state.tr.insertText(text2, from2, to).addMark(from2, from2 + text2.length, mark)
        );
        this.editor.view.focus();
      },
      setTextColor: (color) => {
  ...
  getDynamicParams() {
    ...
    return {
      ...
      textAlignment: this.editor.getAttributes(blockInfo.contentType).textAlignment || "left",
      canIncreaseBlockIndent: this.editor.state.doc.resolve(blockInfo.startPos).index(blockInfo.depth - 1) > 0,
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(transaction, prev, oldState, newState) {
        ...
        if (newState.selection.from !== newState.selection.to || ((_c = transaction.getMeta(pluginKey)) == null ? void 0 : _c.deactivate) || transaction.getMeta("focus") || transaction.getMeta("blur") || transaction.getMeta("pointer") || prev.active && newState.selection.from < prev.queryStartPos || next.notFoundCount > 3) {
          return getDefaultPluginState();
        }
...
class SlashMenuItem {
  constructor(name, execute, aliases = []) {
    this.name = name;
...
class BlockMenuView {
  constructor({
    ...
    __publicField(this, "hoveredBlockContent");
    ...
    this.horizontalPosAnchor = editor.view.dom.firstChild.getBoundingClientRect().x;
    ...
    document.body.addEventListener(
      ...
      (event) => {
        ...
        const editorBoundingBox = this.editor.view.dom.firstChild.getBoundingClientRect();
        ...
        if (this.menuOpen && ((_a = this.hoveredBlockContent) == null ? void 0 : _a.hasAttribute("data-id")) && ((_b = this.hoveredBlockContent) == null ? void 0 : _b.getAttribute("data-id")) === block2.id) {
          return;
        }
        const blockContent2 = block2.node.firstChild;
        this.hoveredBlockContent = blockContent2;
        if (!blockContent2) {
          return;
        }
        if (!this.menuOpen) {
  ...
  addBlock() {
    ...
    this.blockMenu.hide();
    const blockContentBoundingBox = this.hoveredBlockContent.getBoundingClientRect();
  ...
  getStaticParams() {
    return {
      ...
      setBlockBackgroundColor: (color) => this.setBlockBackgroundColor(color),
      setBlockTextColor: (color) => this.setBlockTextColor(color)
    };
  }
  getDynamicParams() {
    const blockContentBoundingBox = this.hoveredBlockContent.getBoundingClientRect();
...
const BackgroundColorMark = Mark.create({
  ...
  addCommands() {
    return {
      setBackgroundColor: (color) => ({ commands: commands2 }) => {
        if (color !== "default") {
          return commands2.setMark(this.name, { color });
        }
        return commands2.unsetMark(this.name);
      }
    };
...
const TextColorMark = Mark.create({
  ...
  addCommands() {
    return {
      setTextColor: (color) => ({ commands: commands2 }) => {
        if (color !== "default") {
          return commands2.setMark(this.name, { color });
        }
        return commands2.unsetMark(this.name);
...
const BackgroundColorExtension = Extension.create({
    ...
    return [
      {
        ...
        attributes: {
          backgroundColor: {
            default: "default",
            parseHTML: (element) => element.hasAttribute("data-background-color") ? element.getAttribute("data-background-color") : "default",
            renderHTML: (attributes) => attributes.backgroundColor !== "default" && {
  ...
  addCommands() {
    return {
      setBlockBackgroundColor: (posInBlock, color) => ({ state, view }) => {
        const blockInfo = getBlockInfoFromPos(state.doc, posInBlock);
        if (blockInfo === void 0) {
          return false;
        }
...
const TextColorExtension = Extension.create({
    ...
    return [
      {
        ...
        attributes: {
          textColor: {
            default: "default",
            parseHTML: (element) => element.hasAttribute("data-text-color") ? element.getAttribute("data-text-color") : "default",
            renderHTML: (attributes) => attributes.textColor !== "default" && {
  ...
  addCommands() {
    return {
      setBlockTextColor: (posInBlock, color) => ({ state, view }) => {
        const blockInfo = getBlockInfoFromPos(state.doc, posInBlock);
        if (blockInfo === void 0) {
          return false;
        }
...
const TextAlignmentExtension = Extension.create({
  ...
  addCommands() {
    return {
      setTextAlignment: (textAlignment) => ({ state }) => {
        ...
        for (const pos2 of positionsBeforeSelectedContent) {
          state.tr.setNodeAttribute(pos2, "textAlignment", textAlignment);
        }
        return true;
      }
    };
